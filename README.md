...
This project will be my self-tutorial courses for learning how to use [BASEMENT v4.2](https://basement.ethz.ch/download/software-download.html) and postprocessing results in ParaView or QGIS. I try to set up, run and postprocessing a few cases. Each case will be stored with its own scripts for pre- and post-processing and input files.
...
### Case 10: Mass movement induced wave

[More details in the case README](./10_massmovement/README.md)

This case demonstrates the new MASSMOVEMENT module introduced in BASEMENT 4.2 by simulating a surface-gravity wave generated by a sudden downthrow of a central movable bottom strip in a long rectangular flume (length 10,000 m, width 50 m, still-water depth 10 m, movable half-width b = 122 m). The settings for this simulation were covered in the series of flume experiment by  Hammack (1973) (see the paper in `10_massmovement/Ref`). The mass-movement thickness is set to 1 m over the movable region ![Schematic_10](/10_massmovement/Physics/Schematic.jpg). The simulation runs from 0–540 s with a 1 s output interval on an unstructured triangular mesh of 51,861 cells and 27,919 nodes.

Comparison of model time series against digitised experiments (probes P1–P4):
![Model_vs_Experiment_10](/10_massmovement/Figures/model_experiment_comparison.png)
